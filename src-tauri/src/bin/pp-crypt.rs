//! `pp-crypt` — passphrase encryption for `pp-box` backup bundles.
//!
//! A backup bundle IS the box: onion key, secrets, agent API keys, transcripts, and the
//! `PP_SECRETS_KEY` that unlocks them. `pp-box backup --encrypt` pipes the bundle through
...
fn passphrase() -> Result<String, String> {
    let p = std::env::var("PP_PASSPHRASE").map_err(|_| "PP_PASSPHRASE is not set".to_string())?;
...
fn open() -> Result<(), String> {
    ...
    let nl = input
        .iter()
        .position(|&b| b == b'\n')
        .ok_or("not a pp-crypt file (no header line)")?;
    let header: serde_json::Value = serde_json::from_slice(&input[..nl])
        .map_err(|_| "not a pp-crypt file (header is not JSON)")?;
    if header.get("ppcrypt").and_then(|v| v.as_u64()) != Some(1) {
        return Err("unsupported pp-crypt version".into());
    }
...
fn main() {
    ...
    let result = match mode.as_str() {
        "seal" => seal(),
        "open" => open(),
        _ => Err("usage: pp-crypt seal|open  (passphrase in $PP_PASSPHRASE, data on stdin)".into()),
    };
    if let Err(e) = result {
        eprintln!("pp-crypt: {e}");
        std::process::exit(1);
